...
- Image filesystem exploring: the web UI shows an image's layers merged into one browsable tree, with the layer that set each entry, a per-layer view of what a layer added, changed or removed, and any file opened inline or downloaded. A durable `index` job walks each layer once and keeps its listing and gzip checkpoints by the layer digest, under new `_angos/layers/<digest>/entries` and `file` endpoints; a repository with `index = true` indexes each image as it lands, and any other image is indexed the first time someone opens its filesystem.
