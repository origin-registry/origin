...
- **Filesystem**: an image manifest shows its layers merged into one tree, the way a runtime applies them: whiteouts remove what lower layers put there, and every entry carries the layer that last set it. A layer filter narrows the tree to what one layer added, changed or removed, a path filter to matching names. Opening a file shows it inline up to 512 KiB, or offers it for download; both go through the registry's layer endpoints, which need the `get-blob` action. The first visit to an image nobody indexed shows "Indexing" until the registry's index jobs have walked its layers.
